...
function wp_authenticate_username_password( $user, $username, $password ) {
	...
	if ( empty( $username ) || empty( $password ) ) {
		...
		if ( empty( $username ) ) {
			$error->add( 'empty_username', __( '<strong>Lỗi:</strong> Trường tên đăng nhập đang trống.', 'monamedia' ) );
		}

		if ( empty( $password ) ) {
			$error->add( 'empty_password', __( '<strong>Lỗi:</strong> Trường mật khẩu đang trống.', 'monamedia' ) );
		}
	...
	if ( ! $user ) {
		return new WP_Error(
			...
			/* translators: %s: Tên người dùng. */
				__( '<strong>Lỗi:</strong> Tên người dùng "<strong>%s</strong>" không được đăng ký trên trang web này. Nếu bạn không chắc chắn về tên người dùng của mình, hãy thử sử dụng địa chỉ email của bạn thay thế.', 'monamedia' ),
				$username
			)
		);
	}
	...
	if ( ! wp_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new WP_Error(
			...
			sprintf(
			/* translators: %s: Tên người dùng. */
				__( '<strong>Lỗi:</strong> Mật khẩu mà bạn nhập cho tài khoản "%s" không chính xác.', 'monamedia' ),
...
function wp_authenticate_email_password( $user, $email, $password ) {
	...
	if ( empty( $email ) || empty( $password ) ) {
		...
		if ( empty( $email ) ) {
			...
			$error->add( 'empty_username', __( '<strong>Lỗi:</strong> Trường email đang trống.', 'monamedia' ) );
		}
	...
	if ( ! $user ) {
		return new WP_Error(
			'invalid_email',
			__( 'Địa chỉ email không tồn tại. Vui lòng kiểm tra lại hoặc thử sử dụng tên đăng nhập của bạn.', 'monamedia' )
		);
	}
	...
	if ( ! wp_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new WP_Error(
			'incorrect_password',
			sprintf(
			/* translators: %s: Địa chỉ email. */
				__( '<strong>Lỗi:</strong> Mật khẩu mà bạn nhập cho địa chỉ email %s không chính xác.', 'monamedia' ),
				'<strong>' . $email . '</strong>'
			) .
			' <a href="' . wp_lostpassword_url() . '">' .
			__( 'Quên mật khẩu?' ) .
